...
export default function ProductDetailsScreen() {
  ...
  const t = {
    errorLoad: isArabic
      ? "فشل تحميل تفاصيل المنتج. يرجى المحاولة لاحقاً"
      : "Failed to load product details. Please try again",
    loginAlert: isArabic
      ? "يجب تسجيل الدخول أولاً لإجراء حجز"
      : "You must login first to make a reservation",
    successTitle: isArabic ? "تم الحجز بنجاح" : "Reservation Successful",
    successMsg: (q: number, n: string) =>
      isArabic
        ? `تم حجز (${q}) من منتج ${n} بنجاح! يمكنك استلامه من الصالون وتفعيله في زيارتك القادمة.`
        : `Successfully reserved (${q}) of ${n}! You can pick it up at the salon.`,
    ok: isArabic ? "موافق" : "OK",
    errorReserve: isArabic
      ? "حدث خطأ أثناء حجز المنتج"
      : "Error occurred while reserving",
    ...
    inStock: (q: number) =>
      isArabic ? `متوفر (${q} قطع متبقية)` : `In Stock (${q} left)`,
  ...
  const handleConfirmReservation = async () => {
    ...
    try {
      ...
      Alert.alert(t.successTitle, t.successMsg(quantity, product.name), [
        {
          text: t.ok,
          onPress: () => {
    ...
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        >
          <Text style={styles.categoryBadge}>
            {product.category || "العناية بالرجل"}
          </Text>
          <Text
            style={[
              styles.productName,
          ...
          >
            <Text
              style={[
                styles.stockLabel,
                { color: theme.textPrimary },
                isArabic ? { marginLeft: 8 } : { marginRight: 8 },
              ]}
            >
              {t.stockLabel}
            </Text>
            <Text
              style={[
                styles.stockValue,
                product.quantityInStock <= 0 && styles.outOfStockText,
              ]}
            >
              {product.quantityInStock > 0
                ? t.inStock(product.quantityInStock)
                : t.outOfStock}
            </Text>
          </View>
        </View>
      ...
      >
        <TouchableOpacity
          style={[
            styles.bookBtn,
            product.quantityInStock <= 0 && styles.disabledBookBtn,
        ...
        >
          ...
          >
            ...
            <View style={styles.modalBody}>
              <Text
                style={[styles.modalProductName, { color: theme.textPrimary }]}
              >
                {product.name}
              </Text>
              <Text
              ...
              >
                <Text
                  style={[styles.quantityLabel, { color: theme.textPrimary }]}
                >
                  {t.modalQuantity}
                </Text>
                <View style={styles.counterRow}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setQuantity((prev) =>
                        Math.min(product.quantityInStock, prev + 1),
                      )
                    }
                  >
                  ...
                  <Text
                    style={[styles.counterVal, { color: theme.textPrimary }]}
                  >
                    {quantity}
                  </Text>
                ...
                {reserving ? (
                  ...
                  <>
                    ...
                    <Ionicons
                      ...
                      size={18}
                      color="#121212"
                      style={isArabic ? { marginRight: 6 } : { marginLeft: 6 }}
                    />
                  </>
                )}
              </TouchableOpacity>
